...
(async () => {
    ...
    const limpias = sitios.sitios.map(
        async (x) => {
    ...
    const publicados = sitios.sitios.filter((s) => s.compilar).map(
        (s) => s.folder + (s.keyVersion ? ":" + s.keyVersion : ""),
    );
    try {
                ...
                return limit(() => {
                    if (x.compilar) {
                        if (!x.customBuild) {
                            ...
                            child =
                                exec(
                                    `${net}\\aspnet_compiler.exe -v /${x.folder} -p svn\\${x.folder} -f built\\${x.folder}${
                                        x.xclude ? (
                                            typeof (x.xclude) === "string" ? " -x " + x.xclude : x.xclude.map(
                                                (xi) => " -x " + xi,
                                            ).join(" ")
                                        ) : ""
                                    }`,
    ...
    if (sitios.vars.comprimir) {
        ...
        const child = exec(
            `"${sitios.vars.z7}" a sitios.7z -mx=3 -bsp1 -y -ms=100m built publicados.json thePublisher.exe thePublisher.exe.config thePublisher.pdb .\\*.dll ${
                (pass !== false) ? " -p" : ""
            }`,
        );
