...
(async () => {
    ...
    if (sitios.vars.comprimir) {
        ...
        const child = exec(
            `"${sitios.vars.z7}" a sitios.7z -mx=3 -y -ms=100m built publicados.json Microsoft.Web.Infrastructure.dll Oracle.ManagedDataAccess.dll Newtonsoft.Json.dll thePublisher.exe thePublisher.exe.config ${(pass !== false)
                ? " -p"
                : ""}`
        );
